...
import * as KeeperUtils from '@nevermined-io/nevermined-sdk-js/dist/node/keeper/utils'
import { Logger, Config } from '@nevermined-io/nevermined-sdk-js'
import { ZeroAddress } from '@nevermined-io/nevermined-sdk-js/dist/node/utils'
...
export default abstract class TestContractHandler extends ContractHandler {
  public static async prepareContracts(config: Config): Promise<string> {
  ...
  private static web3: ethers.providers.JsonRpcProvider
  ...
  private static async deployContracts(deployerAddress: string) {
    ...
    const erc1155 = await TestContractHandler.deployContract('NFTUpgradeable', deployerAddress, [
      '',
    ])
    ...
    const didRegistry = await TestContractHandler.deployContract(
      'DIDRegistry',
      deployerAddress,
      [deployerAddress, erc1155.address, erc721.address, nvmConfig.address, royalties.address],
      {
        DIDRegistryLibrary: didRegistryLibrary.address,
      },
    )

    let transactionResponse: TransactionResponse = await erc1155.addMinter(didRegistry.address)
    ...
    transactionResponse = await erc1155.setProxyApproval(transferNftCondition.address, true)
    contractReceipt = await transactionResponse.wait()
    if (contractReceipt.status !== 1) {
      throw new Error('Error calling "addMinter" on "erc721"')
    }
  ...
  public static async deployArtifact(
    ...
    if (!from) {
      from = (
        await TestContractHandler.addressesStatic(
          TestContractHandler.config,
          TestContractHandler.web3,
        )
      )[0]
  ...
  private static getSignatureOfMethod(
    ...
    const methods = contractInstace.interface.fragments.filter((f) => f.name === methodName)
